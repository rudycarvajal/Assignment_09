...
# Rcarvajal, 2020-Sep-13, Added code to DataProcessor class: select cd and add track methods
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        for row in table:
            if row.cd_id == cd_idx:
                return row
        raise Exception('This CD is not currently in inventory')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        trackPos, trackTitle, trackLength = track_info
        try:
            trackPos = int(trackPos)
        except:
            raise Exception('Position must be an Integer')
        track = DC.Track(trackPos, trackTitle, trackLength)
        cd.add_track(track)
